/*
 * Implements desktop click, drag, and object-activation behavior. The file
 * is the main interaction layer that turns user gestures into desktop
 * operations.
 *
...
EXTERN WORD objc_change(LONG tree, WORD drawob, WORD depth, WORD xc, WORD yc, WORD wc, WORD hc, WORD newstate, WORD redraw);
EXTERN WORD graf_rubbox(WORD xorigin, WORD yorigin, WORD wmin, WORD hmin, WORD *pwend, WORD *phend);
...
*/
WORD gr_isdown(WORD out, WORD x, WORD y, WORD w, WORD h, WORD *pmx, WORD *pmy, WORD *pbutton, WORD *pkstate)
{
...
WORD gr_bwait(GRECT *po, WORD mx, WORD my, WORD numpts, WORD *xylnpts, WORD numobs, WORD *xyobpts)
{
	WORD		down;
	WORD		ret[4];
						/* draw old		*/
...
*/
/* Local behavior fixes kept from the imported code. */
VOID act_bsclick(WORD wh, LONG tree, WORD root, WORD mx, WORD my, WORD keystate, GRECT *pc, WORD dclick)
{
	WORD		obj;
	WORD		shifted;
	WORD		state;
...
*/
WORD act_bdown(WORD wh, LONG tree, WORD root, WORD *in_mx, WORD *in_my, WORD keystate, GRECT *pc, WORD *pdobj)
{
	WORD		sobj;
	WORD		numobs, button;
	OBJECT		*olist;
