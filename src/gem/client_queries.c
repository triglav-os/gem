...
VOID graf_mkstate(WORD *mx, WORD *my, WORD *buttons, WORD *keys)
{
    ...
    if (!gem_rpc_call(GEM_RPC_GRAF_MKSTATE, NULL, 0, NULL,
            &rsp, sizeof(rsp))) {
        return;
    }
    if (mx != NULL) *mx = rsp.values[0];
    if (my != NULL) *my = rsp.values[1];
    if (buttons != NULL) *buttons = rsp.values[2];
    if (keys != NULL) *keys = rsp.values[3];
}
    ...
    WORD distances[], WORD *max_width, WORD effects[])
{
    ...
    if (!gem_rpc_call(GEM_RPC_VQT_FONTINFO, &req, sizeof(req),
            &status, &rsp, sizeof(rsp))) {
        return 0;
    }
    if (min_ade != NULL) *min_ade = rsp.values[0];
    if (max_ade != NULL) *max_ade = rsp.values[1];
    if (distances != NULL) memcpy(distances, &rsp.values[2], 5 * sizeof(WORD));
    if (max_width != NULL) *max_width = rsp.values[7];
    if (effects != NULL) memcpy(effects, &rsp.values[8], 3 * sizeof(WORD));
    return (WORD) status;
}
...
WORD scrp_read(char *path)
{
    ...
    if (path == NULL || !gem_rpc_call(GEM_RPC_SCRP_READ, NULL, 0,
            &status, &rsp, sizeof(rsp))) {
        return 0;
    }
...
WORD scrp_write(char *path)
{
    ...
    if (path == NULL || strlen(path) >= sizeof(req.text)) return 0;
    strcpy(req.text, path);
    if (!gem_rpc_call(GEM_RPC_SCRP_WRITE, &req, sizeof(req),
            &status, NULL, 0)) return 0;
    return (WORD) status;
}
...
WORD fsel_input(char *path, char *name, WORD *button)
{
    ...
    if (path == NULL || name == NULL || button == NULL ||
        ...
        strlen(name) >= sizeof(req.name)) return 0;
    strcpy(req.path, path);
    strcpy(req.name, name);
    if (!gem_rpc_call(GEM_RPC_FSEL_INPUT, &req, sizeof(req),
            &status, &rsp, sizeof(rsp))) return 0;
