/*
 * Implements partial resource loading and import support used by the
 * resource construction set. The file helps the editor merge or reuse
 * pieces of existing resources.
 *
...
else strcpy(sav_icnpath, full_path);
...
/* r e a d _ i c o n							*/
/************************************************************************/
	LONG read_icon(buffer, icon_w, icon_h, data_size, icon_name, mask, icn_size)
	...
	BYTE		*buffer, *icon_name;
{
	...
	*icn_size = ptr - buffer;
	...
	WORD bb_get(taddr, icon_w, icon_h)
	LONG	taddr;
	WORD	*icon_w, *icon_h;
	{
	LONG	where;
	...
	WORD ib_get(taddr, icon_w, icon_h, maskp, file_size)
	LONG	taddr;
...
else LLSET(IB_PMASK(taddr), where);
