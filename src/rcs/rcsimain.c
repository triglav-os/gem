/*
 * Implements the top-level control flow for the resource construction set
 * icon editor. The file coordinates event handling specific to icon-
 * editing mode.
 *
...
EXTERN WORD vsl_ends(WORD begsty, WORD endsty);
EXTERN WORD vsl_width(WORD width);
...
EXTERN WORD graf_dragbox(WORD w, WORD h, WORD sx, WORD sy, WORD xc, WORD yc, WORD wc, WORD hc, WORD *pdx, WORD *pdy);
...
else LWCOPY(LLGET( BI_PDATA(taddr)), buffer, gl_datasize);
...
WORD snap_point(WORD *sx, WORD *sy)
{
...
VOID constrn(WORD sx, WORD sy, WORD *cx, WORD *cy, BOOLEAN line)
{
    WORD	dx, dy;
...
else rast_op(3,&src_area,&scrn_mfdb,&new_area,&scrn_mfdb);
...
else v_ellarc(smlxy[0], smlxy[1], t1pxy[2]>>3, t1pxy[3]>>3, 0, 3600);
            }
	    scr2buf();
	    save_fat(FALSE);
	    gsx_sclip(&fat_area);
