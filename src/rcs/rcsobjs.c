/*
 * Implements object-level editing, selection, and manipulation helpers for
 * the resource construction set. It complements the tree-level editing
 * routines with per-object behavior.
 *
...
EXTERN	WORD	gridw, gridh;
	...
	WORD newsize_obj(tree, obj, neww, newh, ok)
	LONG	tree;
	...
	VOID constrain_obj(tree, obj, px, py)
	LONG	tree;
	WORD	obj, *px, *py;
	{
	WORD	tx, ty, x, y;
	...
	WORD posn_obj(tree, obj, px, py, w, h, exact_ok)
	LONG	tree;
...
/* rectangle must fit, but not exactly, to nest under the obj */
	...
	*py = ty - o.g_y;
	...
	WORD new_obj(stree, sobj, dobj, dx, dy)
	LONG	stree;
...
else obj_redraw(tree, pobj); 
		obj_redraw(tree, obj);
	}	

	VOID slct_obj(tree, obj)
	WORD	obj;
	LONG	tree;
	{
	WORD	nbar, type;
	...
	WORD less_ob(tree, obj1, obj2, mode)
	LONG	tree;
	WORD	obj1, obj2, mode;
	{
	WORD	x1, y1, x2, y2;
