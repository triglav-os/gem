                ...
                WORD *cellh)
{
    ...
    vdi_compat.text_height = (height > 0) ? height : vdi_font_text_height();
    if (charw != NULL) {
        *charw = vdi_font_cell_width();
    }
    if (charh != NULL) {
        *charh = vdi_compat.text_height;
    }
    if (cellw != NULL) {
        *cellw = vdi_font_cell_width();
    }
    if (cellh != NULL) {
        *cellh = vdi_compat.text_height;
    }
    return vdi_compat.text_height;
}
...
WORD vst_rotation(WORD handle, WORD angle)
{
    ...
    vdi_compat.text_rotation = angle;
    return angle;
}
...
WORD vsl_width(WORD handle, WORD width)
{
    ...
    vdi_compat.line_width = (width > 0) ? width : 1;
    return vdi_compat.line_width;
}
...
WORD vsm_height(WORD handle, WORD height)
{
    ...
    vdi_compat.marker_height = (height > 0) ? height : vdi_font_text_height();
    return vdi_compat.marker_height;
}
...
WORD vsm_color(WORD handle, WORD color_index)
{
    ...
    vdi_compat.marker_color = vdi_color_to_pixel(color_index);
    return color_index;
}
...
WORD vst_font(WORD handle, WORD font)
{
    ...
    selected = vdi_select_font(font);
    if (selected == 0) {
        return 0;
    }
    vdi_compat.text_font = font;
    return font;
}
...
WORD vsf_interior(WORD handle, WORD style)
{
    ...
    vdi_compat.fill_interior = style;
    return style;
}
...
WORD vswr_mode(WORD handle, WORD mode)
{
    ...
    vdi_compat.write_mode = mode;
    return mode;
}

WORD vql_attributes(WORD handle, WORD attrib[])
{
    if (!vdi_valid_handle(handle) || attrib == NULL) {
        return 0;
    }

    attrib[0] = vdi_compat.line_style;
    attrib[1] = vdi_state.line_color;
    attrib[2] = vdi_compat.write_mode;
    attrib[3] = vdi_compat.line_width;
    return 1;
}
...
WORD vqm_attributes(WORD handle, WORD attrib[])
{
    ...
    attrib[0] = vdi_compat.marker_type;
    attrib[1] = vdi_compat.marker_color;
    attrib[2] = vdi_compat.write_mode;
    attrib[3] = vdi_compat.marker_height;
    return 1;
}
...
WORD vqf_attributes(WORD handle, WORD attrib[])
{
    ...
    attrib[0] = vdi_compat.fill_interior;
    attrib[1] = vdi_state.fill_color;
    attrib[2] = vdi_compat.fill_style;
    attrib[3] = vdi_compat.fill_perimeter;
    return 1;
}
...
WORD vqt_attributes(WORD handle, WORD attrib[])
{
    ...
    attrib[0] = vdi_compat.text_font;
    attrib[1] = vdi_state.text_color;
    attrib[2] = vdi_compat.text_rotation;
    attrib[3] = vdi_compat.text_height;
    attrib[4] = vdi_compat.text_alignment_h;
    attrib[5] = vdi_compat.text_alignment_v;
    return 1;
}
...
WORD vst_alignment(WORD handle, WORD horiz, WORD vert)
{
    ...
    vdi_compat.text_alignment_h = horiz;
    vdi_compat.text_alignment_v = vert;
    return 1;
}
...
WORD vq_extnd(WORD handle, WORD owflag, WORD work_out[])
{
    ...
    if (!vdi_valid_handle(handle) || work_out == NULL) {
        return 0;
    }
...
WORD vsf_perimeter(WORD handle, WORD per_vis)
{
    ...
    vdi_compat.fill_perimeter = per_vis;
    return per_vis;
}
...
WORD vst_background(WORD handle, WORD mode)
{
    ...
    vdi_compat.text_background = mode;
    return mode;
}
...
WORD vst_effects(WORD handle, WORD effect)
{
    ...
    vdi_compat.text_effects = effect;
    return effect;
}
...
WORD vsl_ends(WORD handle, WORD begstyle, WORD endstyle)
{
    ...
    vdi_compat.line_beg_style = begstyle;
    vdi_compat.line_end_style = endstyle;
    return 1;
}
...
{
    size_t count =
        sizeof(vdi_compat.fill_pattern) / sizeof(vdi_compat.fill_pattern[0]);

    (void)planes;

    if (!vdi_valid_handle(handle) || fill_pat == NULL) {
        return 0;
    }

    memcpy(vdi_compat.fill_pattern, fill_pat, sizeof(WORD) * count);
    return 1;
}
...
WORD vsl_udsty(WORD handle, WORD pattern)
{
    ...
    vdi_compat.line_pattern = pattern;
    return pattern;
}

WORD vqt_extent(WORD handle, char *string, WORD extent[8])
{
    if (!vdi_valid_handle(handle) || extent == NULL) {
        return 0;
    }

    vdi_set_extent_for_string(string, extent);
    return 1;
}
               ...
               WORD *right_delta)
{
    ...
    width = vdi_char_cell_width((char)character);
...
WORD vst_load_fonts(WORD handle, WORD select)
{
    ...
    (void)select;
    return vdi_load_external_fonts();
}
...
WORD vst_unload_fonts(WORD handle, WORD select)
{
    ...
    (void)select;
    vdi_compat.text_font = vdi_default_font_id();
    return vdi_unload_external_fonts();
}
...
WORD vqt_name(WORD handle, WORD element_num, BYTE *name)
{
    ...
    if (!vdi_valid_handle(handle) || name == NULL) {
        return 0;
    }

    font_id = vdi_font_id_for_element(element_num);
    font_name = vdi_font_name(element_num);
                  ...
                  WORD *max_width, WORD effects[])
{
    ...
    if (min_ade != NULL) {
        *min_ade = vdi_font_first_ade();
    }
    if (max_ade != NULL) {
        *max_ade = vdi_font_last_ade();
    }
    if (distances != NULL) {
        distances[0] = 0;
        distances[1] = (WORD)(vdi_font_ascent() - 2);
        distances[2] = 2;
        distances[3] = text_height;
        distances[4] = text_height;
    }
    if (max_width != NULL) {
        *max_width = vdi_font_cell_width();
    }
    if (effects != NULL) {
        effects[0] = 0;
        effects[1] = 0;
...
WORD vq_chcells(WORD handle, WORD *rows, WORD *columns)
{
    ...
    if (rows != NULL) {
        *rows = (WORD)(vdi_state.height / vdi_font_text_height());
    }
    if (columns != NULL) {
        *columns = (WORD)(vdi_state.width / vdi_font_cell_width());
    }
    return 1;
}
