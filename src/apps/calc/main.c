/*
 * Implements a hosted monochrome GEM calculator application. The app
 * uses one AES object tree, redraws only visible regions, and keeps the
 * classic calculator actions available through buttons and keyboard
 * shortcuts without relying on the old accessory trap glue.
 *
...
static void calc_init_tree(calc_state_t *state)
{
    ...
    calc_init_object(&state->tree[calc_root], G_BOX, LASTOB, NORMAL, 0L,
        0, 0, 264, 236);
...
int main(void)
{
    ...
    (void) wind_open(state.handle, 100, 60, 300, 310);
