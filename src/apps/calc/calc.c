/*
 * Implements the calculator accessory UI state, display updates, and
 * arithmetic workflow. The file wires the resource tree to the keypad
 * logic used by the sample GEM application.
 *
...
GLOBAL BYTE	*gl_disp;
GLOBAL WORD	equal_clr;	/* Preserve equal-clear behavior. */
...
/*
*	Initializes the calculator tree, TEDINFO records, and display
*	strings used by the accessory.
*/
LONG ini_calc(VOID)
{
	LONG		tree;
	...
	for (i=0; i<NUM_OBS; i++)
	{
	  ...
	  if (the_calc[i].ob_type == G_BOXTEXT)
	  {
	    ...
	    j++;
					...
					/* Use alternate object colors when the display supports them. */
	if (color == 2)
	  {
...
VOID get_rslt(VOID)
{
	...
	if (((result[0] & 0x7f) <= 63) && 
	    (!(((result[0] & 0x7f) == 63) && 
	     (last_op == TIMES))))		   
	{
	 if ( last_calc != RETMEM )	/* Preserve equal-clear behavior. */
{
	   if (last_op == TIMES)
             temp_pt--;
	   if (last_op == DIVIDE)
...
VOID clr_mem(VOID)
{
	...
	memory[0] = 0;			/* Preserve equal-clear behavior. */
...
VOID mem_op(VOID)
{
	...
	if ( last_calc != NOP )		/* Preserve the imported filename update logic. */
last_calc = EQUAL;
	...
	WORD		ii;
	 ...
	 {  	
	   ...
	   if ((last_calc != EQUAL) && (last_calc != PLUMIN) &&
	       ( last_calc != NOP ))		/* Preserve the imported filename update logic. */
op1[0] = (64 + dec_pt1) | neg1;
	   dec_pt1 =64;
	   if (last_calc == RETMEM)
	     for (ii = 0;ii < 10;op1[ii] = memory[ii++]);
...
VOID mem_sub(VOID)
{
	...
	/* Preserve the imported filename update logic. */
if (((stack_num == 0) || (last_calc == EQUAL) || ( last_calc == NOP )) || 
             ((stack_num == 1) && (last_calc == PLUMIN)))
	 {  	
	   if (dec_pt1 == 64)
...
WORD hndl_calc(WORD obj)
{
	...
	switch(obj)
	{
	  ...
	  case ZERO:
		...
		if ( (last_calc == EQUAL) ||
		     (last_calc == RETMEM) || ( equal_clr ) )  	/* Preserve equal-clear behavior. */
/* Reset calculator */
		  zero_res();
	        if ( (last_calc == NOP) ||
		     (last_calc == EQUAL) || ( equal_clr ) )	/* Preserve equal-clear behavior. */
...
/*		      last_calc = NOP;	DELETE LINE 3/3/86 */
		...
		break;
	  case EQUAL:		
		if ((last_op != EQUAL) && ( last_op != NOP ))	/* Preserve equal-clear behavior. */
{	   
	          if (last_calc == RETMEM)
		    for (ii = 0;ii < 10;op2[ii] = memory[ii++]);	 
elseset_op2(VOID);	
	...
	equal_clr = ( obj == EQUAL );		/* Preserve equal-clear behavior. */
display(&inp_str[0]);
	if ((last_calc == PERCENT) || 
	    (inp_str[0] == 'e'))	
	  clr_calc ();		
