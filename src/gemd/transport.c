...
{
    int result;
    if (io->ready || io->output_size)
        return 0;
    if (!io->header_read)
        io->started = gem_os_ticks_ms();
    result =
        receive_part(fd, &io->header, &io->header_read, sizeof(io->header));
    if (result <= 0)
        return result;
    if (io->header.magic != GEM_RPC_MAGIC ||
        io->header.version != GEM_RPC_VERSION ||
        io->header.size > GEM_RPC_PAYLOAD_MAX)
        return -1;
...
void gemd_reply(gemd_io_t *io, int32_t status, const void *data, uint32_t size)
{
    ...
    if (size)
        memcpy(io->output + sizeof(reply), data, size);
...
int gemd_send(int fd, gemd_io_t *io)
{
    ...
    if (io->output_sent != io->output_size)
        return 0;
    io->output_size = io->output_sent = 0;
    return 1;
}
