...
static WORD aes_build_visible_rects(WORD handle, GRECT out[], WORD max_rects);
...
static void aes_redraw_scrollbar_delta(const aes_window_t *before,
...
WORD wind_get(WORD handle, WORD field, WORD *w1, WORD *w2, WORD *w3, WORD *w4)
{
    ...
    if (handle == 0) {
        ...
        if (aes_state.menu_visible != 0 && aes_state.menu_tree != NULL) {
            ...
            menu_height =
                (box_height > 0) ? box_height : aes_menu_chrome_height();
            if (menu_height <= 0) {
                menu_height = aes_menu_chrome_height();
            }
        } else {
            menu_height = 0;
        }
