...
static WORD aes_menu_popup_container(OBJECT *tree);
static WORD aes_menu_first_popup_child(OBJECT *tree, WORD popup_parent);
static WORD aes_menu_last_object(OBJECT *tree);
static WORD aes_menu_last_reachable_object(const OBJECT *tree, WORD object,
                                           uint8_t visited[256]);
static WORD aes_menu_title_container(OBJECT *tree);
...
static WORD aes_menu_popup_for_title(OBJECT *tree, WORD title);
...
void aes_menu_switch_to_app(WORD app_id)
{
    ...
    if (menu_app == NULL || menu_app->menu_tree == NULL ||
        ...
        menu_app = aes_find_app_by_id(aes_state.desktop_owner_app_id);
    }
    ...
    aes_state.menu_owner_app_id = (menu_app != NULL) ? menu_app->id : 0;
        ...
        while (popup != NIL) {
            ...
            while (child != NIL) {
                ...
                if (tree[child].ob_type == G_STRING && child_spec != 0) {
                    ...
                    if (separator == 0) {
                        rendered_width = (WORD)vdi_string_width(
                            has_shortcut != 0 ? shortcut_label : text);
                        needed_width =
                            (WORD)(tree[child].ob_x + rendered_width + 4 +
                                   popup_inner_right_padding);

                        if (has_shortcut != 0 && shortcut_text[0] != '\0') {
                            needed_width =
                                (WORD)(needed_width +
                                       vdi_string_width(shortcut_text) + 12);
                        }
            ...
            while (child != NIL) {
                tree[child].ob_width =
                    (WORD)aes_max_word((WORD)1, (WORD)(popup_width - 3));
                if (child == tree[popup].ob_tail ||
                    tree[child].ob_next == popup ||
                    tree[child].ob_next == NIL) {
                    break;
