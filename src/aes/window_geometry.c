...
#include <stdlib.h>
#include <string.h>
static WORD aes_window_left_border(const aes_window_t *window);
static int aes_window_has_vscroll(const aes_window_t *window);
static int aes_window_has_hscroll(const aes_window_t *window);
static WORD aes_window_scroll_span(const aes_window_t *window);
static WORD aes_window_scroll_button_side(const aes_window_t *window);
static WORD aes_window_right_border(const aes_window_t *window);
static WORD aes_window_bottom_border(const aes_window_t *window);
static WORD aes_window_title_height(const aes_window_t *window);
static int aes_is_window_work_root(const OBJECT *tree);
static int aes_is_dialog_root_tree(const OBJECT *tree);
static int aes_is_menu_bar_object(const OBJECT *tree, WORD object);
static int aes_is_dialog_frame_object(const OBJECT *tree, WORD object,
                                      WORD parent);
static int aes_window_closer_rect(const aes_window_t *window, GRECT *rect);
static int aes_window_fuller_rect(const aes_window_t *window, GRECT *rect);
static int aes_window_title_rect(const aes_window_t *window, GRECT *rect);
static int aes_window_sizer_rect(const aes_window_t *window, GRECT *rect);
...
static int aes_window_vup_rect(const aes_window_t *window, GRECT *rect);
static int aes_window_vdown_rect(const aes_window_t *window, GRECT *rect);
static int aes_window_hleft_rect(const aes_window_t *window, GRECT *rect);
static int aes_window_hright_rect(const aes_window_t *window, GRECT *rect);
...
static void aes_present_window_frame(const aes_window_t *window);
...
static void aes_fill_rect(WORD x0, WORD y0, WORD x1, WORD y1, WORD color);
static void aes_draw_rect_edges(WORD x0, WORD y0, WORD x1, WORD y1,
                                int draw_top, int draw_right, int draw_bottom,
                                int draw_left);
static void aes_invert_rect(WORD x0, WORD y0, WORD x1, WORD y1);
static void aes_fill_pattern_rect(WORD x0, WORD y0, WORD x1, WORD y1,
                                  const uint8_t *rows, size_t row_count);
static void aes_fill_checker_rect(WORD x0, WORD y0, WORD x1, WORD y1);
int aes_window_is_top(const aes_window_t *window);
static void aes_draw_window_icon(WORD x0, WORD y0, WORD x1, WORD y1,
                                 char glyph);
...
static void aes_draw_trace(const char *fmt, ...);
static void aes_stipple_text_pixels(WORD x, WORD y, WORD foreground,
...
static void aes_draw_text(WORD x, WORD y, WORD color, const char *text);
static void aes_draw_ted_object(const OBJECT *tree, WORD object,
                                const OBJECT *obj, const TEDINFO *ted,
                                const WORD rect[4]);
static void aes_draw_hline(WORD x0, WORD x1, WORD y, WORD color);
static void aes_draw_vline(WORD x, WORD y0, WORD y1, WORD color);
static void aes_draw_dialog_frame(const WORD rect[4]);
static void aes_draw_button_frame(const WORD rect[4], WORD dark_color,
                                  WORD light_color, int default_button);
static void aes_draw_icon_object(const OBJECT *obj, const ICONBLK *icon,
                                 WORD abs_x, WORD abs_y, const WORD rect[4]);
static void aes_draw_user_object(const OBJECT *tree, WORD object,
                                 const OBJECT *obj, WORD abs_x, WORD abs_y,
                                 WORD clip[4]);
...
void aes_redraw_region(const GRECT *dirty)
{
    ...
    if (aes_state.menu_visible != 0 && aes_state.menu_tree != NULL) {
        ...
        if (bar != NIL &&
            aes_menu_subtree_rect(aes_state.menu_tree, bar, &menu_rect) != 0 &&
            aes_rects_intersect(&menu_rect, dirty) != 0) {
            redraw_menu = 1;
        }
    }
                            ...
                            WORD abs_y, WORD clip[4])
{
    ...
    checked_menu_item =
        (obj->ob_type == G_STRING && (obj->ob_state & CHECKED) != 0u) ? 1 : 0;
    ...
    } else if (obj->ob_type == G_STRING && tree == aes_state.menu_tree &&
               parent != NIL &&
               (tree[parent].ob_type == G_BOX ||
    ...
    if ((obj->ob_type == G_STRING || obj->ob_type == G_TITLE ||
        ...
        if (checked_menu_item != 0) {
            aes_draw_text((WORD)(rect[0] + 2), text_y, text_color, "\010");
        }
        aes_draw_text(text_x, text_y, text_color, text);
        if (disabled_text != 0) {
