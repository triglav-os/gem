...
static WORD aes_form_last_object(OBJECT *tree);
static WORD aes_form_last_reachable_object(const OBJECT *tree, WORD object,
                                           uint8_t visited[256]);
static int aes_form_is_editable(const OBJECT *tree, WORD object);
static WORD aes_form_find_next_editable(OBJECT *tree, WORD current,
                                        int backwards);
static void aes_form_object_rect(OBJECT *tree, WORD object, GRECT *rect);
static void aes_form_redraw_tree(OBJECT *tree);
static void aes_form_redraw_object(OBJECT *tree, WORD object);
static void aes_form_set_active_field(OBJECT *tree, WORD object, WORD *idx);
static void aes_form_set_caret_from_click(OBJECT *tree, WORD object,
...
static int aes_form_apply_key(OBJECT *tree, WORD object, WORD key, WORD *idx);
