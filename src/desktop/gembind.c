/*
 * Implements GEM binding wrappers used by the desktop shell. These helpers
 * sit between desktop code and the underlying AES and VDI service calls.
 *
...
WORD form_keybd(LONG form, WORD obj, WORD nxt_obj, WORD thechar, WORD *pnxt_obj, WORD *pchar)
{
	FM_FORM = form;
	FM_OBJ = obj;
	FM_INXTOB = nxt_obj;
...
WORD form_button(LONG form, WORD obj, WORD clks, WORD *pnxt_obj)
{
	...
	*pnxt_obj = FM_ONXTOB;
	return( RET_CODE );
					...
					/* Process Manager		*/
WORD proc_create(LONG ibegaddr, LONG isize, WORD isswap, WORD isgem, WORD *onum)
{
	WORD	ret;
...
WORD proc_run(WORD proc_num, WORD isgraf, WORD isover, LONG pcmd, LONG ptail)
{
	PR_NUM = proc_num;
	PR_ISGRAF = isgraf;
	PR_ISOVER = isover;
...
WORD proc_info(WORD num, WORD *oisswap, WORD *oisgem, LONG *obegaddr, LONG *ocsize, LONG *oendmem, LONG *ossize, LONG *ointtbl)
{
	WORD		ret;
...
WORD proc_shrink(WORD pid)
{
	...
	return(gem_if(PROC_SHRINK));
					...
					/* Graphics Manager		*/
WORD graf_rubbox(WORD xorigin, WORD yorigin, WORD wmin, WORD hmin, WORD *pwend, WORD *phend)
{
	GR_I1 = xorigin;
	GR_I2 = yorigin;
	GR_I3 = wmin;
