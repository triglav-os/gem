...
static LONG demo29_draw_user(LONG parm_long)
{
    ...
    gauge[2] = (WORD)(fill[0] + 4 + ((parm->pb_w - 9) * state->level) / 10);
...
static void init_tree(demo29_state_t *state)
{
    ...
    init_object(&state->tree[D29_ROOT], G_IBOX, NONE, NORMAL, 0L, 0, 0, 340,
                204);
    init_object(&state->tree[D29_HEAD], G_STRING, NONE, NORMAL,
                (LONG)(intptr_t) "Demo 29 boxchar and user objects", 14, 14,
                220, 8);
    ...
    init_object(&state->tree[D29_STATUS], G_STRING, NONE, NORMAL, 0L, 14, 116,
                280, 8);
    init_object(&state->tree[D29_WATCH], G_BUTTON, SELECTABLE | EXIT, NORMAL,
                (LONG)(intptr_t) "Watchbox", 14, 142, 88, 22);
    init_object(&state->tree[D29_RUBBOX], G_BUTTON, SELECTABLE | EXIT, NORMAL,
                (LONG)(intptr_t) "Rubbox", 112, 142, 72, 22);
    init_object(&state->tree[D29_DONE], G_BUTTON, SELECTABLE | EXIT | DEFAULT,
                NORMAL, (LONG)(intptr_t) "Done", 266, 142, 58, 22);
    ...
    state->tree[D29_DONE].ob_flags |= LASTOB;
...
static void probe_watchbox(demo29_state_t *state)
{
    ...
    objc_draw(state->tree, ROOT, MAX_DEPTH, rect.g_x, rect.g_y, rect.g_w,
              rect.g_h);
}
...
static void handle_boxchar(demo29_state_t *state, WORD object)
{
    ...
    sprintf(text, "G_BOXCHAR object %d clicked, level now %d.", object,
            state->level);
