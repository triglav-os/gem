...
enum { D26_DIALOG_ROOT = 0, D26_DIALOG_TEXT, D26_DIALOG_OK, D26_DIALOG_COUNT };
...
OBJECT menu_tree[] = {
    ...
    {5, -1, -1, G_TITLE, NONE, NORMAL, (LONG) " Windows ", 6, 0, 9, 1},
    {2, -1, -1, G_TITLE, NONE, NORMAL, (LONG) " Dialog ", 15, 0, 8, 1},
    {0, 7, 15, G_IBOX, NONE, NORMAL, 0L, 0, 0, 0, 0},
    {11, 8, 10, G_BOX, NONE, NORMAL, 0x1100L, 0, 0, 20, 4},
    {9, -1, -1, G_STRING, NONE, NORMAL, (LONG) "  About Demo 26 ", 0, 0, 20, 1},
    {10, -1, -1, G_STRING, NONE, NORMAL, (LONG) "  ------------ ", 0, 1, 20, 1},
    {7, -1, -1, G_STRING, NONE, NORMAL, (LONG) "  Quit ", 0, 2, 20, 1},
    {15, 12, 14, G_BOX, NONE, NORMAL, 0x1100L, 0, 0, 22, 4},
    {13, -1, -1, G_STRING, NONE, NORMAL, (LONG) "  Top main window ", 0, 0, 22,
     1},
    {14, -1, -1, G_STRING, NONE, NORMAL, (LONG) "  Top log window ", 0, 1, 22,
     1},
    {11, -1, -1, G_STRING, NONE, NORMAL, (LONG) "  Toggle log window ", 0, 2,
     22, 1},
    {6, 16, 16, G_BOX, NONE, NORMAL, 0x1100L, 0, 0, 18, 2},
    {15, -1, -1, G_STRING, LASTOB, NORMAL, (LONG) "  Show dialog ", 0, 0, 18,
     1}};
...
void init_dialog(void)
{
    ...
    dialog_tree[D26_DIALOG_ROOT].ob_flags = NONE;
    ...
    dialog_tree[D26_DIALOG_OK].ob_flags = LASTOB | SELECTABLE | EXIT | DEFAULT;
...
void draw_window(WORD handle, const char *message, GRECT *clip)
{
    ...
    while (box.g_w > 0 && box.g_h > 0) {
        ...
        if (x0 <= x1 && y0 <= y1) {
            ...
            v_gtext(vdi_handle, (WORD)(work.g_x + 16), (WORD)(work.g_y + 28),
                    (BYTE *)message);
        }
...
void show_dialog(void)
{
    ...
    while (1) {
        ...
        if (object == D26_DIALOG_OK) {
            ...
            objc_change(dialog_tree, D26_DIALOG_OK, 0, rect.g_x, rect.g_y,
                        rect.g_w, rect.g_h, (WORD)(old_state | SELECTED), 1);
            evnt_timer(90, 0);
            objc_change(dialog_tree, D26_DIALOG_OK, 0, rect.g_x, rect.g_y,
                        rect.g_w, rect.g_h, old_state, 1);
            break;
        }
    }
...
void open_log_window(void)
{
    ...
    log_handle =
        wind_create(NAME | CLOSER | FULLER | MOVER | SIZER, full_rect.g_x,
                    full_rect.g_y, full_rect.g_w, full_rect.g_h);
    if (log_handle < 0) {
        return;
    }

    wind_set_str(log_handle, WF_NAME, "Demo 26 Log");
    wind_open(log_handle, log_normal.g_x, log_normal.g_y, log_normal.g_w,
              log_normal.g_h);
    log_open = 1;
}
...
int main(void)
{
    ...
    wind_set_str(main_handle, WF_NAME, "Demo 26 Main");
    wind_set_str(log_handle, WF_NAME, "Demo 26 Log");
