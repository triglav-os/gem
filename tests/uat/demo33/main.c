...
static WORD centered_text_baseline(WORD top, WORD height, WORD text_height,
                                   WORD text_ascent)
{
    WORD pad = 0;
...
static void init_dialog_tree(WORD text_height, WORD text_ascent)
{
    ...
    init_object(&dialog_tree[D33_ROOT], G_IBOX, NONE, NORMAL, 0L, 0, 0, 420,
                220);
    init_object(&dialog_tree[D33_PANEL], G_BOX, NONE, NORMAL, 0L, 0, 0, 420,
                220);
    ...
    init_object(&dialog_tree[D33_HINT], G_STRING, NONE, NORMAL,
                (LONG)(intptr_t) "Tab moves forward. Click places caret. Enter "
                                 "accepts.",
                18, 30, 320, 8);
    ...
    for (i = 0; i < D33_FIELD_COUNT; ++i) {
        ...
        WORD baseline =
            centered_text_baseline(y, field_height, text_height, text_ascent);
        ...
        init_object(&dialog_tree[label_id], G_STRING, NONE, NORMAL,
                    ...
                    label_height);
        init_object(&dialog_tree[field_id], field_type, SELECTABLE | EDITABLE,
                    NORMAL, (LONG)(intptr_t)&ted[i], field_x, field_y, 118,
                    field_height);
        objc_add(dialog_tree, D33_ROOT, label_id);
        objc_add(dialog_tree, D33_ROOT, field_id);
    }

    init_object(&dialog_tree[D33_OK], G_BUTTON, SELECTABLE | EXIT | DEFAULT,
                NORMAL, (LONG)(intptr_t) "OK", 282, 184, 56, 22);
    init_object(&dialog_tree[D33_CANCEL], G_BUTTON, SELECTABLE | EXIT, NORMAL,
                (LONG)(intptr_t) "Cancel", 346, 184, 64, 22);
    objc_add(dialog_tree, D33_ROOT, D33_OK);
    objc_add(dialog_tree, D33_ROOT, D33_CANCEL);
    dialog_tree[D33_CANCEL].ob_flags |= LASTOB;
}
...
int main(void)
{
    ...
    if (result == D33_OK) {
        snprintf(alert, sizeof(alert), "[1][ Saved %s / %s ][ OK ]", buffers[0],
                 buffers[2]);
        form_alert(1, alert);
