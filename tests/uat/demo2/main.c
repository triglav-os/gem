...
static void draw_font_cell(VDI_HANDLE handle, WORD left, WORD top, WORD right,
                           WORD bottom, WORD font_id, const BYTE *font_name)
{
    WORD clip[4];
    char label[96];
...
int main(void)
{
    ...
    cell_width =
        (WORD)((width - demo2_margin * 2 - demo2_gutter * (demo2_columns - 1)) /
               demo2_columns);
    cell_height = (WORD)((height - demo2_margin * 2 - demo2_title_height -
                          demo2_gutter * (rows - 1)) /
                         rows);
