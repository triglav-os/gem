 ...
 * backend. The sample renders a simple scrollback terminal with a
 * blinking block cursor, forwards keyboard input to the shell, and
 * maps the window scroll bars onto the terminal viewport.
 *
...
enum {
    DEMO31_MAX_LINES = 2048,
    DEMO31_MAX_COLS = 512,
    DEMO31_MARGIN = 2,
    DEMO31_ROW_TIGHTEN = 4,
    DEMO31_TIMER_MS = 50,
    DEMO31_KEY_FLUSH_MS = 30,
    DEMO31_READ_BUF = 512
};

typedef struct demo31_state {
    WORD handle;
    WORD char_w;
    WORD char_h;
    WORD cell_w;
...
static void ensure_row(demo31_state_t *state, WORD row)
{
    WORD i;

    while (row >= DEMO31_MAX_LINES) {
...
static void recompute_max_line_len(demo31_state_t *state)
{
    WORD row;
    WORD best = 0;
...
static void set_char_at_cursor(demo31_state_t *state, char ch)
{
    char *line;
    WORD len;
    WORD i;

    if (state->cursor_col >= DEMO31_MAX_COLS) {
        state->cursor_col = 0;
        ++state->cursor_row;
...
static void newline(demo31_state_t *state)
{
    state->cursor_col = 0;
    ++state->cursor_row;
    ensure_row(state, state->cursor_row);
...
static void process_shell_byte(demo31_state_t *state, unsigned char byte)
{
...
static void follow_bottom(demo31_state_t *state)
{
    state->scroll_row = max_offset(state->total_rows, state->visible_rows);
    state->scroll_col = 0;
}

static int process_shell_output(demo31_state_t *state, GRECT *dirty)
{
    char buf[DEMO31_READ_BUF];
    int32_t count;
    int i;
    int changed = 0;
...
static void send_key(demo31_state_t *state, WORD key)
{
    unsigned char ch;
...
static int init_terminal(demo31_state_t *state)
{
    WORD distances[5];
...
static void shutdown_terminal(demo31_state_t *state)
{
