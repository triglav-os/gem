 ...
 * with other independent GEM processes (see tests/uat/demo18). Ported
 * from tests/uat/demo31: renders a simple scrollback terminal with a
    ...
    while (box.g_w > 0 && box.g_h > 0) {
        ...
        if (x0 <= x1 && y0 <= y1) {
            ...
            for (row = 0; row <= state->visible_rows; ++row) {
                ...
                v_gtext(vdi_handle, (WORD)(state->work.g_x + TERM_MARGIN),
...
static int init_terminal(term_state_t *state)
{
    ...
    state->cells = gem_os_alloc((size_t)TERM_MAX_LINES * (TERM_MAX_COLS + 1u));
    if (state->cells == NULL) {
        return 0;
    }
    memset(state->cells, 0, (size_t)TERM_MAX_LINES * (TERM_MAX_COLS + 1u));
    ...
    if (!gem_os_pty_spawn_shell(&state->pty,
                                getenv("GEM_SHELL") ? getenv("GEM_SHELL")
                                                    : "/bin/bash",
                                cwd, 80, 25)) {
